...
struct AppIntentsMetadataProcessorCaptureGroup: CaptureGroup {
    static let outputType: OutputType = .task
    static let regex = XCRegex(pattern: #"^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d+ appintentsmetadataprocessor\[\d+:\d+\] (.+)$"#)
...
struct ConstructStubExecutorLinkFileListCaptureGroup: CaptureGroup {
    static let outputType: OutputType = .task
    static let regex = XCRegex(pattern: #"^ConstructStubExecutorLinkFileList (.+) \(in target '(.+)' from project '(.+)'\)$"#)
...
struct GenerateTAPICaptureGroup: CaptureGroup {
    static let outputType: OutputType = .task

    static let regex = XCRegex(pattern: #"^GenerateTAPI (.+) \(in target '(.+)' from project '(.+)'\)$"#)
...
struct LinkAssetCatalogCaptureGroup: CaptureGroup {
    static let outputType: OutputType = .task
    static let regex = XCRegex(pattern: #"^LinkAssetCatalog (.+) \(in target '(.+)' from project '(.+)'\)$"#)
